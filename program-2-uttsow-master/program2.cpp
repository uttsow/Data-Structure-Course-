...
int main(){
	...
	//####################Your Tests#####################//
	cerr << "\n\tTEST #9: 1st tests of your own" << endl;

	Table newGame(2, ante);
	Player a("Uttsow", 10);
	Player b("Bob", 0);
	...
	assert(b.bet(ante) == false);

	//This will test if player b can bet. The results should be false since his budget is
	//lower than the ante.
	cerr << "\n\t========================PASS========================\n" << endl;
	cerr << "\n\tTEST #9: 2nd tests of your own" << endl;


	Heap heap1("players2.txt");
	Table game2(3, 3);
	while(game2.emptySeat()){
		game2.addPlayer(heap1.getPlayer());
	}

	while(!heap1.empty()){
		while(!heap1.empty() && game2.emptySeat()){
			game2.addPlayer(heap1.getPlayer());
		}


	vector<Player> v = game2.playRound();
	...
	//this shows that there is only 1 winner per round. It prints out the winners of each round
